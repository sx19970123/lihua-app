  ...
  methods: {
    ...
    getVerifyData() {
      ...
      uni.request({
        ...
        success: ({ data }) => {
			const d = data.data
			this.captchaData.id = d.id;
			this.captchaData.background_image = d.backgroundImage;
			this.captchaData.slider_image = d.templateImage;
			this.type = d.type;
			this.$nextTick(() => {
			  // 获取当前设备上渲染的背景图和滑块尺寸
			  ...
			  if (this.type === "ROTATE" || this.type === "SLIDER") {
				  uni.createSelectorQuery().in(this).select('#slider-img').boundingClientRect(resp => {
					  this.sliderImg.width = resp.width;
					  this.sliderImg.height = resp.height;
				  }).exec()
			    ...
			    //   })
			    //   .exec();
				uni.createSelectorQuery().in(this).select('#image-click-mask').boundingClientRect(resp => {
					this.sliderImg.left = resp.left;
					this.sliderImg.top = resp.top;
				}).exec()
			  }
			  setTimeout(() => {
    ...
    setVertifyData() {
      const dataForm = {
        ...
        data: {
          ...
          startTime: this.captchaData.startTime.getTime(),
          stopTime: this.captchaData.stopTime.getTime(),
